...
class CallApi(object):
    def getCallResult(self, pageSize, pageNumber, phoneNumber, request):
        try:
            data={}
            if request == None:
                now = datetime.now()
    ...
    def getTag(self, request):
        # phoneList=json.loads(const.PHONE_LIST)
        phoneList=[]
        if request != None and request.args.get('session_id') != None:
            phoneList=["0277550100"]
        else:
            phoneList=["0277550100","0277550101","0277550102"]
        totalResult = []
        for phone in phoneList:
            totalSize=0
            resultForTotalSize = self.getCallResult(1,1,phone,request)
            #calculate necessary call api number of times
            totalSize=resultForTotalSize['result']['total_size']
            log.debug("Get tag by number:" + phone + ",total_size:" + str(totalSize))
            try:
                result = self.getCallResult(totalSize,1,phone,request)
                if result['status']!=0:
    ...
    def transmitToArms(self,dataForRakuten):
        log.info("transmitToArms:" + dataForRakuten['data'][0]['session_id'])
